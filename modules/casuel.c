...
void add_casuel() {
    char input[50];
    printf("|| Saisissez vos casuels (entrez 'q' pour quitter):\n");
    while (1) {
        printf("|| Montant: ");
        fflush(stdout); 
        scanf("%s", input);
        ...
        FILE *f = fopen(CASUEL_FILE, "a");
        if (!f) { perror("Erreur ouverture fichier casuels"); return; }
        fprintf(f, "%s,%s,%.2f\n", date, heure, montant);
        fclose(f);
...
void afficher_casuels(time_t date_ref) {
    ...
    do { 
        ...
        // Lecture fichier
        FILE *f = fopen(CASUEL_FILE, "r");
        if (!f) { 
            printf("  Aucune casuel enregistree.\n"); 
            return; 
        }

        char line[256]; 
        double total = 0.0;
        int casuels_trouvees = 0;
        ...
        if (casuels_trouvees == 0) {
            printf("   Aucune casuel cette semaine.\n");
        } else {  
            printf("----------------------------------------------\n\n");
            printf("%-21s|%-4sSYNTHESE GENERALE\n", spc, spc);
            printf("----------------------------------------------\n");
            printf("Nombre :%-13s|%-3s%3d casuels \n", spc, spc, casuels_trouvees);
            printf("Total  :%-13s|%-3s\033[1;36m%8.2f\033[0m dhs\n", spc, spc, total);
            printf("----------------------------------------------\n");
        }
        ...
        switch (choix_semaine) {
            ...
            case 1: choix_semaine = 4; add_casuel(); break; 
            case 3: 
                printf("\n\033[3J\033[H\033[2J");
                choix_semaine = 4;
                afficher_bilan(); 
